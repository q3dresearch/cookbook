...
| 39 | **When does the market work out which reports were right?** | `answered` · *the first attempt used the wrong instrument* | **A fixed 30-day window cannot answer this and looked like noise because it was noise.** The median target takes **454 days** to file its restatement or delisting notice, so a month-long window measures a period in which nothing has happened yet. Running each target from its report to its OWN filing date instead: median **−45% against the market over a median 454 days, 14 of 19 negative**. The windowed chart is kept as the negative example | `report-to-outcome` |
| 40 | **Does being right pay?** | `answered — and less reliably than being right` | **Five of nineteen vindicated targets BEAT the market between the report and the filing that conceded the point** — Bloom Energy by 194%, MX by 107%, CG by 70%. The wait runs from 20 days to six years. Being right about a company is a claim about the company; being paid is a claim about timing, and on this corpus they are close to unrelated | `report-to-outcome` |
| 41 | **Was anything in the windowed version worth keeping?** | `yes, one thing` | **Before the report, the doomed companies were holding up (+0.9%) while the eventual survivors were already sliding (−6.9%).** That is backwards from intuition and survives the window criticism, because it does not depend on catching an outcome inside 30 days — it is about the month before, which is fully observed | `before-and-after` |

...
| 47 | **Which jackpot do they actually hit — the crime or the broken business model?** | `answered — the best-powered result here` | **The crime, decisively; the business model, not at all.** Against 139 controls each given a report date drawn from the target distribution so exposure matches: a target discloses a Wells notice, grand jury, subpoena or formal order of investigation within three years at **13/59 = 22% against 6/108 = 6%, a 4.0× lift at Fisher p = 0.0021**. On the economics side nothing reaches significance and **targets have BETTER gross margins than random filers** — median +41% against +28%, with negative gross margins *rarer* among targets (2% vs 6%). Negative operating margin 49% vs 34% (p=0.20), cash burn 37% vs 24% (p=0.16). So the firms are not finding companies whose unit economics fail; they are finding companies about to be investigated. **Held with one caution: a short report can cause the investigation it appears to predict**, and nothing here separates the two | `two-jackpots` |

...
| 39 | **When does the market work out which reports were right?** | `answered — and it is the sharpest result in the recipe` | **Roughly a month after, never on the day.** Targets that later filed a restatement, delisting notice or bankruptcy moved **−2.8% on the report; those that did not, −2.7%** — indistinguishable. Over the following month they split by fifteen points: **−12.8% against +2.0%**. And before the report the doomed ones were the ones holding up (**+0.9% against −6.9%**), which is backwards from intuition. Size explains none of it: correlation between log market cap and the post-report return is +0.03 | `before-and-after` |

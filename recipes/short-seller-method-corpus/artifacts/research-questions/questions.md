...
| 47 | **Which jackpot do they actually hit — the crime or the broken business model?** | `answered — the best-powered result here` | **The crime, at every size; the business model, not at all.** Pooled against 139 exposure-matched controls: investigation disclosed within three years **13/59 = 22% vs 6/108 = 6%, 4.0× at p = 0.0021**; economics composite 30/61 = 49% vs 19/58 = 33%, 1.5× at p = 0.093. Fit against log public float, one logistic per group: **neither crime slope is resolvable** (+0.06 ±0.31 targets, +0.48 ±0.27 controls) while **both economics slopes are steeply negative** (−2.21 ±0.61, −0.95 ±0.41). So the pooled economics lift is the size mix, not the targeting, and the crime gap holds across a thousandfold range of market value. Targets even have **better gross margins** than random filers, +41% vs +28%, negative gross margins rarer (2% vs 6%) | `two-jackpots` |
| 48 | **Should size be banded, or continuous with a spline?** | `answered — banding was manufacturing a shape` | **Continuous, and linear — the event count forbids a spline.** Banded into three filer classes the target crime rate read 21%, 9%, 28%: a U-shape a reader will believe, eleven companies in the middle bin. The fitted slope is +0.06 ±0.31 — there is no shape. But the continuous version cannot be flexible either: **ten crime events among targets and five among controls**, and at ten-events-per-parameter that supports about one parameter while a two-parameter logistic already spends two. A three-knot spline is four parameters against five events and will draw a confident wiggle every time. Even the fitted line is dashed where the slope cannot be told from zero, because the control curve climbs toward 60% on five events that all sit between $800m and $16bn | `two-jackpots` |
| 49 | **Is total assets a usable size axis?** | `tested and failed` | **No, twice over.** XBRL returns values in the filer's own currency and an earlier capture took whichever unit came first, which put **Nomura Holdings in the data at 62.6 trillion — yen, about $400bn in reality**. And for a bank `Assets` is the loan book, so Bank of America at $3.5tn outweighs every industrial in the corpus by two orders of magnitude. `dei:EntityPublicFloat` is a US regulatory disclosure — always USD, always market value — at the cost of coverage, since foreign private issuers do not file it (21 of 76 targets). The fits run on assets give the same signs and verdicts, so the conclusion survives the swap | — *(the failure is the finding; `capture_profiles.py` now refuses non-USD units)* |

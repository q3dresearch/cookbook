...
| 47 | **Which jackpot do they actually hit — the crime or the broken business model?** | `answered — the best-powered result here` | **The crime, decisively; the business model, not really.** Against 139 controls each given a report date drawn from the target distribution so exposure matches: a target discloses a Wells notice, grand jury, subpoena or formal order of investigation within three years at **13/59 = 22% against 6/108 = 6%, a 4.0× lift at Fisher p = 0.0021**, and the 95% intervals do not touch. The economics composite — does not cover overhead, or burns cash — reads 30/61 = 49% against 19/58 = 33%, 1.5× at p = 0.093, intervals overlapping across most of their range. **And that lean is mostly composition:** split by filer size class it nearly vanishes, 31% vs 20% among large accelerated filers and 85% vs 79% among everything smaller. Small companies fail to cover overhead four times in five whether or not anyone shorts them, and targets skew large while controls skew small. Targets even have **better gross margins** than random filers, +41% vs +28%, with negative gross margins rarer (2% vs 6%). **Held with one caution: a short report can cause the investigation it appears to predict**, and nothing here separates the two | `two-jackpots` |

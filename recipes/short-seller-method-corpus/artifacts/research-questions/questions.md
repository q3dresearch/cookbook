...
| 30 | **What happened to the companies they targeted?** | `answered — but the label lies` | Of 32 targets confirmed by two independent routes, years after their report: **59% still trade under the same symbol, 25% are gone from it, 12% changed symbol, 3% are bankrupt.** Reading the 13 non-survivors dissolves the categories — KDNY became NVS because Chinook Therapeutics was **acquired by Novartis**, a premium exit sitting in the same bucket as a fraud. SQ→XYZ and AMEH→ASTH are renames of thriving firms. Among the "gone", RINO and CIFS are delisted Chinese frauds but FMCN went private and relisted in China, WSP trades on the TSX, IRSA is still on the NYSE | — *(no figure: the measure does not mean what a chart of it would claim)* |
| 31 | **Can a kill be told apart from a takeover?** | `blocked — and it is the question worth answering` | What Q30 measures is symbol persistence on a US venue, not corporate death. Separating them needs corporate actions — merger, bankruptcy and deregistration filings, all of which SEC keeps for companies that have since deregistered. That is a bounded next step and the natural pair to Q29: an accuracy axis needs an outcome, and an outcome needs this | — |
...
| **Whether a target died or was bought** | 41% of confirmed targets no longer trade under the symbol their report named, and that number mixes delisted frauds with companies acquired at a premium. Symbol persistence is not corporate survival; separating them needs merger and bankruptcy filings |
